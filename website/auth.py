...
def choose_template():
    if request.method=='POST':
        ...
        db.session.commit()
        pf=Personalinfo(workid=workspace.workid,ids=str(uuid.uuid4()),name="Laura",phone=2345167287,email="xyz@abc.com",freelance="Available",degree="master",age=34,designation="Web designer, Photographer",yoex=5,project=6,hpclients=345,awards=76,about="Lorem ipsum dolor sit, amet consectetur adipisicing elit. Itaque delectus sit architecto magni nesciunt aut quasi pariatur minus rem sequi voluptate perspiciatis quas hic tenetur quae blanditiis molestiae, omnis, beatae ut. Blanditiis veniam tenetur quisquam!")
        db.session.add(pf)
        db.session.commit()
        sk=Skill(workid=workspace.workid,ids=str(uuid.uuid4()),skill="html",perc=89)
        db.session.add(sk)
        db.session.commit()
        edu=Education(workid=workspace.workid,ids=str(uuid.uuid4()),course="Computer Science",year="2000-present",descr="Lorem ipsum dolor sit, amet consectetur adipisicing elit. Itaque delectus sit architecto magni nesciunt aut quasi pariatur minus rem sequi voluptate perspiciatis quas hic tenetur quae blanditiis molestiae, omnis, beatae ut. Blanditiis veniam tenetur quisquam!",clg="iit kharagpur")
        db.session.add(edu)
        db.session.commit()
        exp=Experience(workid=workspace.workid,ids=str(uuid.uuid4()),title="Frontend Developer",year="2000-present",descr="Lorem ipsum dolor sit, amet consectetur adipisicing elit. Itaque delectus sit architecto magni nesciunt aut quasi pariatur minus rem sequi voluptate perspiciatis quas hic tenetur quae blanditiis molestiae, omnis, beatae ut. Blanditiis veniam tenetur quisquam!",company="google inc.")
        db.session.add(exp)
        db.session.commit()
